...
$TemplateFolderName = 'Office Templates' # Folder name on desktop when files willbe downloaded
    ...
    if ($CustomPowerPointTemplatesLocation -ne $null)
        {
            Write-Host "PersonalTemplates reg exist in HKCU:\SOFTWARE\Microsoft\Office\16.0\Excel\Options"
        }
        else {
...
$xml = Invoke-WebRequest -Method Get -Uri "https://$storageAccountName/$containerName/?comp=list" -ContentType "application/xml" -UseBasicParsing # Get All files in Blob Container without SAS token
